...
- [x] T081 [P] 에러 상태 UI — 네트워크 에러, 404, 빈 목록 EmptyState 컴포넌트 (`src/shared/ui/EmptyState.tsx`, `src/app/not-found.tsx`)
- [x] T082 [P] ErrorBoundary 적용 — 페이지 레벨 렌더링 에러 선언적 처리 (`src/shared/ui/ErrorBoundary.tsx`, 각 `src/app/*/error.tsx`)
- [x] T083 quickstart.md 체크포인트 전체 수동 검증 실행
